...
module.exports = async function handler(req, res) {
  ...
  const images = imagesBase64.slice(0, 5).map(s => s.replace(/^data:image\/\w+;base64,/, ''));

  const systemPrompt = `You are analyzing ${images.length} recent photos of a ${plantType} named "${plantName}" to synthesize its general current condition for a garden map display. Average across all photos to capture consistent, typical appearance — ignore transient moments.
